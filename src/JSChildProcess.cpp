...
static JSValue ExecSync(JSContext* ctx, JSValueConst this_val, int argc,
                        JSValueConst* argv) {
  if (argc < 1) {
    return JS_EXCEPTION;
  }
  ...
  si.hStdOutput = write;
  si.hStdError = write;
  if (!CreateProcess(NULL, const_cast<LPSTR>(cmd.To<std::string>().c_str()),
                     NULL, NULL, TRUE, DETACHED_PROCESS, NULL, NULL, &si,
                     &pi)) {
...
static JSValue Exec(JSContext* ctx, JSValueConst this_val, int argc,
                    JSValueConst* argv) {
  if (argc < 1) {
    return JS_EXCEPTION;
  }
